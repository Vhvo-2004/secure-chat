...
async function promptForMissingCredentials(options) {
	...
	try {
		...
		if (!options.authSource) {
			const authSource = await question(
...
async function purgeCollections(connection) {
	...
	if (!collections.length) {
		console.log(
			'[reset-db] No collections found to purge. Database is already empty.',
		);
		return;
	}
...
async function main() {
	...
	try {
		...
		if (isAuthError(error)) {
			console.error(
				'[reset-db] Authentication failed when connecting to MongoDB.',
			);
			console.error(
			...
			console.error(
				'[reset-db] Unexpected error while purging the database.',
			);
			console.error(error);
		}
		process.exitCode = 1;
		...
		try {
			...
			if (disconnectError) {
				console.error(
					'[reset-db] Failed to disconnect cleanly:',
