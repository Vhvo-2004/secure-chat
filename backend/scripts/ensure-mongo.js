	...
	return {
		...
		cleanup: async () => {
			try {
				...
				console.error(
					'[startup] Failed to stop mongodb-memory-server cleanly.',
				);
				console.error(error);
			}
		},
...
async function ensureMongo() {
	...
	try {
		console.log(
			'[startup] MongoDB is not reachable. Attempting to start docker compose service "mongodb"...',
		);
		...
		console.error(
			'[startup] MongoDB did not become ready in time. Falling back to in-memory instance.',
		);
	} catch (error) {
		const help = [
		...
		console.error(
			'[startup] Failed to auto-start MongoDB using docker compose. Falling back to in-memory instance.',
		);
		if (error) {
		...
		if (error && error.message?.includes('permission denied')) {
			help.unshift(
				'Docker reported a permission error when pulling or starting mongo: grant docker access or run the command with sufficient privileges.',
			);
		}
...
if (require.main === module) {
	ensureMongo()
		...
		.catch((error) => {
			console.error(
				'[startup] Unexpected error while preparing MongoDB.',
			);
			console.error(error);
			process.exitCode = 1;
		});
