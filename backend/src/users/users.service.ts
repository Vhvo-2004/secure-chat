...
export class UsersService {
	...
	async create(payload: CreateUserDto): Promise<User> {
		...
		const normalizedPreKeys = oneTimePreKeys.map((item) =>
			this.mapPreKey(item),
		);
	...
	async addPreKeys(id: string, payload: AddPreKeysDto): Promise<User> {
		...
		const incoming = payload.oneTimePreKeys.map((item) =>
			this.mapPreKey(item),
		);
		const existingIndexes = new Set(
			user.oneTimePreKeys.map((k) => k.index),
		);
		const filtered = incoming.filter(
			(item) => !existingIndexes.has(item.index),
		);
	...
	async reservePreKey(
		userId: string,
		consumerId?: string,
	): Promise<{ user: User; preKey: OneTimePreKey | null }> {
		...
		const plainPreKey =
			plainUser.oneTimePreKeys.find(
				(item) => item.index === available.index,
			) ?? null;
