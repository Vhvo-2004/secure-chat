...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
	...
	handleIdentify(@MessageBody() userId: string, client: Socket): void {
		...
		console.log(
			`Cliente ${client.id} identificado como usuário: ${userId}`,
		);
	}
	...
	sendMessageToUser(recipientId: string, message: any) {
		...
		if (socketId) {
			console.log(
				`Enviando mensagem para ${recipientId} no socket ${socketId}`,
			);
