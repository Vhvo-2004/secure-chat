...
export function useSecureChat() {
  ...
  function handleLogin(username) {
    ...
    const storedBundle = safeJsonParse(localStorage.getItem(`${STORAGE_KEYS.bundle}${username}`), null);
    const storedKeys = safeJsonParse(localStorage.getItem(`${STORAGE_KEYS.groupKeys}${username}`), {});
    const storedUser = safeJsonParse(localStorage.getItem(`${STORAGE_KEYS.user}${username}`), null);
    if (username && storedUser) setCurrentUserData(storedUser);
    if (storedBundle) setPrivateBundle(deserializePrivateBundle(storedBundle));
    if (storedKeys) setGroupKeys(storedKeys);
    setUsernameInput('');
  ...
  async function handleCreateGroup(evt) {
    ...
    const targetMembers = Array.from(new Set([...groupMemberSelections, currentUserData.id]));
