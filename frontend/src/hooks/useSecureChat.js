...
export function useSecureChat() {
  ...
  useEffect(() => {
    ...
    const storedCurrentUser = safeJsonParse(sessionStorage.getItem(STORAGE_KEYS.currentUser), null);
    setCurrentUser(storedCurrentUser);
    const storedBundle = safeJsonParse(
      localStorage.getItem(`${STORAGE_KEYS.bundle}${storedCurrentUser}`),
      null,
    );
    const storedKeys = safeJsonParse(
      localStorage.getItem(`${STORAGE_KEYS.groupKeys}${storedCurrentUser}`),
      {},
    );
    const storedUser = safeJsonParse(
      localStorage.getItem(`${STORAGE_KEYS.user}${storedCurrentUser}`),
      null,
    );
    if (storedCurrentUser && storedUser) setCurrentUserData(storedUser);
  ...
  function storeGroupKey(username, groupId, keyInput, options = {}) {
    ...
    appendCryptoLog({
      ...
      artifacts: [
        ...
        ...(options.showKey === false ? [] : [{ label: 'Chave 3DES (base64)', value: keyB64 }]),
  ...
  function handleLogin(username) {
    ...
    if (!trimmed) {
      setStatus('Informe um nome de usuário válido.');
      return { success: false, message: 'Informe um nome de usuário válido.' };
    }
    const searchResult = searchUser(trimmed);
    if (!searchResult) {
      setStatus('Identidade não encontrada neste navegador. Registre-se primeiro.');
      return {
        success: false,
        message: 'Identidade não encontrada neste navegador. Registre-se primeiro.',
      };
    }
    sessionStorage.setItem(STORAGE_KEYS.currentUser, JSON.stringify(trimmed));
    setCurrentUser(trimmed);
    const storedBundle = safeJsonParse(localStorage.getItem(`${STORAGE_KEYS.bundle}${trimmed}`), null);
    const storedKeys = safeJsonParse(localStorage.getItem(`${STORAGE_KEYS.groupKeys}${trimmed}`), {});
    const storedUser = safeJsonParse(localStorage.getItem(`${STORAGE_KEYS.user}${trimmed}`), null);
    if (trimmed && storedUser) setCurrentUserData(storedUser);
    if (storedBundle) setPrivateBundle(deserializePrivateBundle(storedBundle));
    if (storedKeys) setGroupKeys(storedKeys);
    setUsernameInput('');
    setStatus('Identidade detectada com sucesso.');
    refreshUsers();
    return { success: true, user: storedUser ?? searchResult };
  }
  ...
  async function registerUser(username) {
    ...
    if (!trimmed) {
      ...
      return { success: false, message: 'Informe um nome de usuário.' };
    }
    const already = handleLogin(trimmed);
    if (already.success) {
      return {
        success: true,
        reused: true,
        message: 'Identidade já existente carregada a partir do armazenamento local.',
        user: already.user,
      };
    }
    setIsBusy(true);
    try {
      ...
      appendCryptoLog({
        ...
        description:
          'Criamos pares de chaves de identidade, assinatura, signed pre-key e 10 one-time pre-keys.',
  ...
  async function createGroup(groupName, memberIds = []) {
    if (!currentUserData || !privateBundle) {
      setStatus('Cadastre-se e carregue suas chaves antes de criar grupos.');
      return { success: false, message: 'Cadastre-se e carregue suas chaves antes de criar grupos.' };
    }
    const trimmedName = typeof groupName === 'string' ? groupName.trim() : '';
    if (!trimmedName) {
      setStatus('Informe um nome para o grupo.');
      return { success: false, message: 'Informe um nome para o grupo.' };
    }
    const targetMembers = Array.from(new Set([...(memberIds ?? []), currentUserData.id]));
    if (targetMembers.length < 2) {
      setStatus('Selecione pelo menos mais um participante.');
      return { success: false, message: 'Selecione pelo menos mais um participante.' };
    }
    setIsBusy(true);
    try {
      ...
      const normalizedGroup = await fetchJson(`${API_BASE}/groups`, {
        method: 'POST',
        body: JSON.stringify(groupPayload),
      });
      const groupId = extractId(normalizedGroup);
      storeGroupKey(currentUserData.username, groupId, keyB64, {
        phase: 'Chaves de grupo',
        title: 'Chave 3DES criada localmente',
        description: `Fingerprint ${fingerprint} registrada para o grupo ${groupPayload.name}.`,
        groupName: normalizedGroup.name ?? groupPayload.name,
        artifacts: [
          { label: 'Chave base64', value: keyB64 },
        ],
      });
          ...
          appendCryptoLog({
            ...
            artifacts: [
              ...
              { label: 'Root key (base64)', value: toB64(rootKeyBytes) },
              {
                label: 'OPK utilizada',
                value:
                  packet.opk_index !== null && packet.opk_index !== undefined ? packet.opk_index : 'Nenhuma',
              },
      ...
      return { success: true, group: { ...normalizedGroup, id: groupId } };
    } catch (err) {
  ...
  async function handleCreateGroup(evt) {
    evt?.preventDefault();
    const result = await createGroup(groupNameInput, groupMemberSelections);
    if (result.success) {
      setGroupNameInput('');
  ...
  async function sendMessage(content) {
    if (!currentUserData || !selectedGroupId) {
      setStatus('Selecione um grupo para enviar mensagens.');
      return { success: false, message: 'Selecione um grupo para enviar mensagens.' };
    }
    ...
    if (!key) {
      ...
      return { success: false, message: 'Aceite a chave do grupo antes de enviar mensagens.' };
    }
    try {
      const { ciphertext, iv } = encryptMessage3DES(trimmed, key);
      appendCryptoLog({
        phase: 'Mensagens',
        ...
        artifacts: [
          ...
          { label: 'IV', value: iv },
          { label: 'Mensagem original', value: trimmed },
        ],
      });
  ...
  function handleShare(shareId) {
    return acceptShare(shareId);
  }

  async function acceptShare(shareId) {
    const user = currentUser;
    return handleAcceptShare(shareId, user);
  }
  ...
  async function handleAcceptShare(shareId, username) {
    if (!currentUserData || !privateBundle) {
      return {
        success: false,
        message: 'Cadastre sua identidade local para aceitar convites.',
      };
    }
    ...
    try {
      const { rootKeyBytes, payload, usedOpkIndex, resolvedWithFallback } =
        await performX3DHResponderAndDecrypt(privateBundle, share.packet);
      ...
      storeGroupKey(username, groupId, plainBytes, {
        ...
        artifacts: [{ label: 'Fingerprint esperada', value: share.group?.keyFingerprint ?? '—' }],
      });
